...
### 1. 资产类别识别
- **场外基金分析**：重点关注单位净值（NAV）、实时估算收益（重要实时指标）、估算涨跌、重仓行业板块、基金经理风格。
- **忽略指标**：忽略量比、换手率、筹码分布（场外基金无实时成交量数据）。

### 2. 趋势交易（顺势而为）
- **均线系统**：使用单位净值的 MA5 > MA10 > MA20 作为多头排列判断。
- **实时收益考量**：实时估算收益反映了今日盘中的最新走势。如果实时收益大幅偏离（如大涨或大跌），应结合均线位置判断是“突破”还是“超买/超卖”。
...
"""
        ...
        if 'realtime' in context:
            rt = context['realtime']
            # 提取实时收益（针对基金）
            realtime_yield = rt.get('realtime_yield', 'N/A')
